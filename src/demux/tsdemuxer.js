...
class TSDemuxer {
    ...
    push(data, audioCodec, videoCodec, timeOffset, cc, level, sn, duration, accurateTimeOffset) {
        ...
        if (cc !== this.lastCC) {
            Log.v(this.TAG, 'discontinuity detected');
            this.insertDiscontinuity();
            this.lastCC = cc;
        }
        if (level !== this.lastLevel) {
            Log.v(this.TAG, 'level switch detected');
            this.switchLevel();
                ...
                switch (pid) {
                    ...
                    case pmtId: {
                        ...
                        if (unknownPIDs && !pmtParsed) {
                            Log.v(this.TAG, 'reparse from beginning');
                            unknownPIDs = false;
        ...
        if (audioData && (pes = parsePES(audioData))) {
            ...
            if (audioData && audioData.size) {
                Log.v(this.TAG, 'last AAC PES packet truncated,might overlap between fragments');
            }
    ...
    _parsePAT(data, offset) {
        ...
        //Log.v(this.TAG, 'PMT PID:'  + this._pmtId);
    }
    ...
    _parsePMT(data, offset, mpegSupported) {
        ...
        while (offset < tableEnd) {
            ...
            switch (data[offset]) {
                ...
                case 0x03:
                case 0x04:
                    //Log.v(this.TAG, 'MPEG PID:'  + pid);
                    if (!mpegSupported) {
                        Log.v(this.TAG, 'MPEG audio found, not supported in this browser for now');
                    ...
                    break;
                case 0x24:
                    Log.w(this.TAG, 'HEVC stream type found, not supported for now');
                    break;
                default:
                    Log.v(this.TAG, 'unkown stream type:' + data[offset]);
                    break;
            }
    ...
    pushAccesUnit(avcSample, avcTrack) {
        ...
        if (avcSample.debug.length) {
            Log.v(this.TAG, avcSample.pts + '/' + avcSample.dts + ':' + avcSample.debug + ',' + avcSample.units.length);
        }
    }

    _parseAVCPES(pes, last) {
        //Log.v(this.TAG, 'parse new PES');
        ...
        units.forEach(unit => {
            switch (unit.type) {
                ...
                case 5:
                    ...
                    if (!avcSample) {
                        avcSample = this._createAVCSample(true, pes.pts, pes.dts, '');
                        this.avcSample = avcSample;
                    }
                    if (debug) {
                        avcSample.debug += 'IDR ';
                    }
                ...
                case 6: {
                    ...
                    while (!endOfCaptions && expGolombDecoder._total_bytes > 1) {
                        payloadType = 0;
                        do {
                            b = expGolombDecoder.readByte();
                            payloadType += b;
                        } while (b === 0xFF);
                        ...
                        if (payloadType === 4 && expGolombDecoder._total_bytes !== 0) {

                            endOfCaptions = true;

                            const countryCode = expGolombDecoder.readByte();
                            ...
                            if (countryCode === 181) {
                                ...
                                if (providerCode === 49) {
                                    ...
                                    if (userStructure === 0x47413934) {
                                        const userDataType = expGolombDecoder.readByte();

                                        // Raw CEA-608 bytes wrapped in CEA-708 packet
                                        if (userDataType === 3) {
                                            const firstByte = expGolombDecoder.readByte();
                                            const secondByte = expGolombDecoder.readByte();
                                            ...
                                            for (i = 0; i < totalCCs; i++) {
                                                // 3 bytes per CC
                                                byteArray.push(expGolombDecoder.readByte());
                                                byteArray.push(expGolombDecoder.readByte());
                                                byteArray.push(expGolombDecoder.readByte());
                                            }
                        ...
                        else if (payloadSize < expGolombDecoder._total_bytes) {
    ...
    _parseAVCNALu(array) {
        ...
        //Log.v(this.TAG, 'PES:' + Hex.hexDump(array));
        while (i < len) {
            value = array[i++];
                ...
                case 3:
                    if (value === 0) {
                        ...
                        if (lastUnitStart >= 0) {
                            ...
                            //Log.v(this.TAG, 'pushing NALU, type/size:' + unit.type + '/' + unit.data.byteLength);
                            units.push(unit);
                        } else {
                            ...
                            if (lastUnit) {
                                ...
                                overflow = i - state - 1;
                                if (overflow > 0) {
                                    //Log.v(this.TAG, 'first NALU found with overflow:' + overflow);
                        ...
                        if (i < len) {
                            ...
                            //Log.v(this.TAG, 'find NALU @ offset:' + i + ',type:' + unitType);
                            lastUnitStart = i;
    ...
    _parseAACPES(pes) {
        ...
        if (aacOverFlow) {
            ...
            //Log.v(this.TAG, `AAC: append overflowing ${aacOverFlow.byteLength} bytes to beginning of new PES`);
            data = tmp;
        }
        ...
        if (offset) {
            ...
            Log.w(this.TAG, `parsing error:${reason}`);
            this.observer.trigger(Event.ERROR, {
                type: ErrorTypes.MEDIA_ERROR,
                id: this.id,
                details: ErrorDetails.FRAG_PARSING_ERROR,
        ...
        if (!track.audiosamplerate) {
            ...
            track.duration = this._duration;
            Log.v(this.TAG, `parsed codec:${track.codec},rate:${config.samplerate},nb channel:${config.channelCount}`);
        }
        frameIndex = 0;
        ...
        if (aacOverFlow && aacLastPTS) {
            ...
            if (Math.abs(newPTS - pts) > 1) {
                Log.v(this.TAG, `AAC: align PTS for overlapping frames by ${Math.round((newPTS - pts) / 90)}`);
                pts = newPTS;
            }
        }
        ...
        while ((offset + 5) < len) {
            ...
            let bool = !!(data[offset + 1] & 0x01);
            headerLength = bool ? 7 : 9;
            // retrieve frame size
            ...
            if ((frameLength > 0) && ((offset + headerLength + frameLength) <= len)) {
                ...
                //Log.v(this.TAG, `AAC frame, offset/length/total/pts:${offset+headerLength}/${frameLength}/${data.byteLength}/${(stamp/90).toFixed(0)}`);
                aacSample = {
        ...
        if (offset < len) {
            ...
            //Log.v(this.TAG, `AAC: overflow detected:${len-offset}`);
        } else {
            aacOverFlow = null;
        }
    ...
    _parseMPEGPES(pes) {
        let data = pes.data, pts = pes.pts, length = data.length, frameIndex = 0, offset = 0, parsed;
    ...
    _onMpegFrame(data, bitRate, sampleRate, channelCount, frameIndex, pts) {
        const frameDuration = (1152 / sampleRate) * 1000;
    ...
    _onMpegNoise(data) {
        Log.w(this.TAG, 'mpeg audio has noise: ' + data.length + ' bytes');
    }
